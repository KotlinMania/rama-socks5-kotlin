# Claude Code Project Instructions — rama-socks5-kotlin

## Project Overview

This is **rama-socks5-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-socks5`](https://crates.io/crates/rama-socks5).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-socks5/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-socks5>
